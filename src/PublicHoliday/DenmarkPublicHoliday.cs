...
namespace PublicHoliday
{
    ...
    public class DenmarkPublicHoliday : PublicHolidayBase
    {
        ...
        private DateTime? InternalChristmasEve(int year) => _includeChristmasEve ? ChristmasEve(year) : (DateTime?)null;
        ...
        private DateTime? InternalNewYearsEve(int year) => _includeNewYearsEve ? NewYearsEve(year) : (DateTime?)null;
        ...
        public override IDictionary<DateTime, string> PublicHolidayNames(int year)
        {
            ...
            var allHolidays = new List<KeyValuePair<DateTime?, string>>()
            {
                ...
                new KeyValuePair<DateTime?,string>(InternalChristmasEve(year), "Juleaftensdag"),
        ...
        public override bool IsPublicHoliday(DateTime dt)
        {
            ...
            switch (dt.Month)
            {
                ...
                case 12:
                    if (InternalChristmasEve(year) == date)
                        return true;
                    if (Christmas(year) == date)
                        return true;
                    if (BoxingDay(year) == date)
                        return true;
                    if (InternalNewYearsEve(year) == date)
                        return true;
                    break;
            }
